...
ENTRY_MODE = "entry_geometry_wedge_breakdown_short_1h"
ENTRY_TAG = "geometry_wedge_breakdown_short_1h"
SIDE = "short"
TIMEFRAME = "1h"
...
class Sieve1GeometryWedgeBreakdownShort1H(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        _ = metadata
        dataframe = add_pattern_geometry_v2(dataframe, timeframe=self.timeframe, output_slots=1, include_triangle_patterns=False, include_wedge_patterns=True, include_compression_patterns=False, include_rectangle_patterns=False, include_ascending_channel_patterns=False, include_descending_channel_patterns=False, min_pattern_bars=int(self.min_pattern_bars.value), max_pattern_bars=int(self.max_pattern_bars.value), compression_max_width_atr=float(self.compression_max_width_atr.value), squeeze_active_width_atr=float(self.squeeze_active_width_atr.value), min_line_score=float(self.min_line_score.value), min_containment=float(self.min_containment.value), output_prefix="pg2")
        return dataframe
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        condition = _bool(dataframe, "pg2_wedge_pattern_present") & _bool(dataframe, "pg2_wedge_squeeze_active") & _num(dataframe, "pg2_wedge_indicator_score").ge(float(self.min_line_score.value)) & _num(dataframe, "pg2_wedge_direction").le(0) & _num(dataframe, "close").lt(_num(dataframe, "pg2_wedge_lower", np.nan))
...
apply_explicit_hyperopt_surface(Sieve1GeometryWedgeBreakdownShort1H)
