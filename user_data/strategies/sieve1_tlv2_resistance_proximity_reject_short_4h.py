...
ENTRY_MODE = "entry_tlv2_resistance_proximity_reject_short_4h"
ENTRY_TAG = "tlv2_resistance_proximity_reject_short_4h"
SIDE = "short"
TIMEFRAME = "4h"
...
class Sieve1TLV2ResistanceProximityRejectShort4H(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    pivot_strength = tagged_parameter(IntParameter(2, 4, default=2, space="buy", optimize=False, load=True))
    min_output_line_score = tagged_parameter(DecimalParameter(0.35, 0.80, decimals=2, default=0.50, space="buy", optimize=False, load=True))
    min_output_active_bars = tagged_parameter(IntParameter(2, 24, default=8, space="buy", optimize=False, load=True))
    max_active_line_distance_atr_mult = tagged_parameter(DecimalParameter(3.0, 10.0, decimals=1, default=6.0, space="buy", optimize=False, load=True))
    proximity_rank_weight = tagged_parameter(DecimalParameter(0.00, 0.20, decimals=2, default=0.05, space="buy", optimize=False, load=True))
    proximity_rank_distance_cap_atr = tagged_parameter(DecimalParameter(3.0, 10.0, decimals=1, default=6.0, space="buy", optimize=False, load=True))
    min_anchor_bars = tagged_parameter(IntParameter(6, 24, default=10, space="buy", optimize=False, load=True))
    max_anchor_bars = tagged_parameter(IntParameter(30, 120, default=50, space="buy", optimize=False, load=True))
    max_slope_atr_per_bar = tagged_parameter(DecimalParameter(0.15, 0.70, decimals=2, default=0.35, space="buy", optimize=False, load=True))
    max_projection_bars = tagged_parameter(IntParameter(30, 160, default=50, space="buy", optimize=False, load=True))
    score_min = tagged_parameter(CategoricalParameter([0.5, 0.55, 0.72, 0.82], default=0.5, space="buy", optimize=False, load=True))
    distance_atr_max = tagged_parameter(DecimalParameter(0.15, 3.00, decimals=2, default=0.85, space="buy", optimize=False, load=True))
    line_buffer_pct = tagged_parameter(DecimalParameter(0.000, 0.020, decimals=3, default=0.003, space="buy", optimize=False, load=True))
    line_slope_min_pct = tagged_parameter(DecimalParameter(0.000, 0.010, decimals=4, default=0.001, space="buy", optimize=False, load=True))
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe = add_trendline_projection_v2(
            ...
            min_output_line_score=float(self.min_output_line_score.value),
            min_output_active_bars=int(self.min_output_active_bars.value),
            max_active_line_distance_atr_mult=float(self.max_active_line_distance_atr_mult.value),
            proximity_rank_weight=float(self.proximity_rank_weight.value),
            proximity_rank_distance_cap_atr=float(self.proximity_rank_distance_cap_atr.value),
            min_anchor_bars=int(self.min_anchor_bars.value),
            max_anchor_bars=int(self.max_anchor_bars.value),
            max_slope_atr_per_bar=float(self.max_slope_atr_per_bar.value),
            max_projection_bars=int(self.max_projection_bars.value),
    ...
    def _common_guards(self, dataframe: DataFrame) -> Series:
        ...
        if bool(self.use_ema_trend_guard.value):
            fast_period = int(self.ema_fast_period.value)
            slow_period = int(self.ema_slow_period.value)
            fast = close.ewm(span=fast_period, adjust=False, min_periods=max(2, fast_period // 2)).mean()
            slow = close.ewm(span=slow_period, adjust=False, min_periods=max(2, slow_period // 2)).mean()
            guard &= fast.le(slow) if SIDE == "short" else fast.ge(slow)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        condition = _num(dataframe, "tlv2_resistance_score_rank0").ge(float(self.score_min.value)) & _num(dataframe, "tlv2_resistance_distance_atr_rank0", np.nan).le(float(self.distance_atr_max.value)) & _num(dataframe, "high").ge(line.mul(1.0 - float(self.line_buffer_pct.value))) & _num(dataframe, "close").lt(line) & _num(dataframe, "close").lt(_num(dataframe, "open"))
...
apply_explicit_hyperopt_surface(Sieve1TLV2ResistanceProximityRejectShort4H)
