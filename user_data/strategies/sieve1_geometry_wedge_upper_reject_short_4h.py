...
ENTRY_MODE = "entry_geometry_wedge_upper_reject_short_4h"
ENTRY_TAG = "geometry_wedge_upper_reject_short_4h"
SIDE = "short"
TIMEFRAME = "4h"
...
class Sieve1GeometryWedgeUpperRejectShort4H(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        upper = _num(dataframe, "pg2_wedge_upper", np.nan)
        condition = _bool(dataframe, "pg2_wedge_pattern_present") & _num(dataframe, "pg2_wedge_indicator_score").ge(float(self.score_min.value)) & _num(dataframe, "pg2_wedge_width_atr", np.nan).le(float(self.width_atr_max.value)) & _num(dataframe, "high").ge(upper.mul(1.0 - float(self.rail_buffer_pct.value))) & _num(dataframe, "close").lt(upper) & _num(dataframe, "close").lt(_num(dataframe, "open"))
...
apply_explicit_hyperopt_surface(Sieve1GeometryWedgeUpperRejectShort4H)
