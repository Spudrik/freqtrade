...
ENTRY_MODE = "entry_geometry_wedge_lower_reclaim_long_8h"
ENTRY_TAG = "geometry_wedge_lower_reclaim_long_8h"
SIDE = "long"
TIMEFRAME = "8h"
...
class Sieve1GeometryWedgeLowerReclaimLong8H(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        condition = _bool(dataframe, "pg2_wedge_pattern_present") & _num(dataframe, "pg2_wedge_indicator_score").ge(float(self.score_min.value)) & _num(dataframe, "pg2_wedge_width_atr", np.nan).le(float(self.width_atr_max.value)) & _num(dataframe, "low").le(lower.mul(1.0 + float(self.rail_buffer_pct.value))) & _num(dataframe, "close").gt(lower) & _num(dataframe, "close").gt(_num(dataframe, "open"))
...
apply_explicit_hyperopt_surface(Sieve1GeometryWedgeLowerReclaimLong8H)
