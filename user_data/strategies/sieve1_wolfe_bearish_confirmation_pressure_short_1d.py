...
ENTRY_MODE = "entry_wolfe_bearish_confirmation_pressure_short_1d"
ENTRY_TAG = "wolfe_bearish_confirmation_pressure_short_1d"
SIDE = "short"
TIMEFRAME = "1d"
...
class Sieve1WolfeBearishConfirmationPressureShort1D(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        _ = metadata
        dataframe = add_pattern_wolfe_waves(dataframe, min_wave_quality=float(self.score_min.value))
        return dataframe
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        level = _num(dataframe, "pww_bearish_confirmation_level", np.nan).mul(1.0 - float(self.confirmation_buffer_pct.value))
        condition = _bool(dataframe, "pww_bearish_pattern_present") & _num(dataframe, "pww_bearish_indicator_score").ge(float(self.score_min.value)) & _cross_below(_num(dataframe, "close"), level)
...
apply_explicit_hyperopt_surface(Sieve1WolfeBearishConfirmationPressureShort1D)
