...
ENTRY_MODE = "entry_multi3_pivot_stl_vp_confluence_short"
ENTRY_TAG = "multi3_pivot_stl_vp_confluence_short"
SIDE = "short"
...
class Sieve1Multi3PivotStlVpConfluenceShort(IStrategy):
    """
    ...
    def _pivot_signal(self, dataframe: DataFrame) -> Series:
        ...
        if mode == "break_reclaim":
            signal = _or_columns(dataframe, ["pa_entry_support_breakdown_short", "pa_entry_resistance_reject_short"])
        elif mode == "trend_break":
            signal = _or_columns(dataframe, ["pa_entry_bearish_trend_flip_breakout_short", "pa_entry_bearish_trend_aligned_breakout_short"])
        elif mode == "compression_range":
            signal = _or_columns(dataframe, ["pa_entry_compression_breakdown_short", "pa_entry_range_resistance_short"])
        elif mode == "score_context":
            signal = self._score_context_signal(dataframe, "pa", "short", float(self.pivot_score_min.value), float(self.pivot_context_min.value))
        else:
            signal = _bool(dataframe, "pa_entry_any_short")
        if bool(self.use_pivot_target_guard.value):
            signal &= _num(dataframe, "pa_short_target_distance_pct").ge(float(self.pivot_target_distance_min.value))
        return signal

    def _stl_signal(self, dataframe: DataFrame) -> Series:
        mode = str(self.stl_batch.value)
        if mode == "line_event":
            return _or_columns(dataframe, ["stl_entry_support_breakdown_short", "stl_entry_resistance_reject_short"])
        if mode == "touch_reclaim_reject":
            return _bool(dataframe, "stl_resistance_reject")
        if mode == "triangle":
            return _bool(dataframe, "stl_entry_triangle_breakdown_short") | _num(dataframe, "stl_triangle_score").ge(float(self.stl_triangle_min.value))
        if mode == "score_context":
            return self._stl_score_context_signal(dataframe, "short")
        return _bool(dataframe, "stl_suggested_entry_short")

    def _vp_signal(self, dataframe: DataFrame) -> Series:
        mode = str(self.vp_batch.value)
        if mode == "node":
            return _bool(dataframe, "vp_node_entry_short")
        if mode == "value_area":
            return _or_columns(dataframe, ["vp_val_breakdown_with_pressure", "vp_upper_rejection_with_pressure"])
        if mode == "lvn_hvn":
            return _or_columns(dataframe, ["vp_hvn_above_reject", "vp_lvn_above_reject_short", "vp_lvn_accept_short", "vp_lvn_fast_traverse_short"])
        if mode == "score_context":
            return self._score_context_signal(dataframe, "vp", "short", float(self.vp_score_min.value), float(self.vp_context_min.value))
        return _bool(dataframe, "vp_entry_trigger_short")

    def _volume_guard(self, dataframe: DataFrame) -> Series:
        return _num(dataframe, "entry_volume_ratio").ge(float(self.volume_ratio_min.value)) & _num(dataframe, "entry_pressure").le(-float(self.pressure_min.value))

    def _trend_guard(self, dataframe: DataFrame) -> Series:
        return _num(dataframe, "entry_ema_fast").le(_num(dataframe, "entry_ema_slow"))
...
apply_explicit_hyperopt_surface(Sieve1Multi3PivotStlVpConfluenceShort)
