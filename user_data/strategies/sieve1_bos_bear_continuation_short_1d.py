...
ENTRY_MODE = "entry_bos_bear_continuation_short_1d"
ENTRY_TAG = "bos_bear_continuation_short_1d"
SIDE = "short"
TIMEFRAME = "1d"
...
class Sieve1BOSBearContinuationShort1D(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    strength = tagged_parameter(IntParameter(2, 5, default=3, space="buy", optimize=False, load=True))
    min_prominence_atr = tagged_parameter(DecimalParameter(0.20, 1.20, decimals=2, default=0.35, space="buy", optimize=False, load=True))
    min_pivot_spacing_bars = tagged_parameter(IntParameter(1, 8, default=2, space="buy", optimize=False, load=True))
    max_pivot_age_bars = tagged_parameter(IntParameter(24, 160, default=96, space="buy", optimize=False, load=True))
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        _ = metadata
        dataframe = add_bos_choch(dataframe, strength=int(self.strength.value), min_prominence_atr=float(self.min_prominence_atr.value), min_pivot_spacing_bars=int(self.min_pivot_spacing_bars.value), max_pivot_age_bars=int(self.max_pivot_age_bars.value), breakout_buffer_atr=float(self.breakout_buffer_atr.value), prefix="ms")
        return dataframe
...
apply_explicit_hyperopt_surface(Sieve1BOSBearContinuationShort1D)
