...
ENTRY_MODE = "entry_continuation_pennant_present_short_1h"
ENTRY_TAG = "continuation_pennant_present_short_1h"
SIDE = "short"
TIMEFRAME = "1h"
...
class Sieve1ContinuationPennantPresentShort1H(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    rail_buffer_pct = tagged_parameter(CategoricalParameter([0.0, 0.003, 0.005, 0.01, 0.02], default=0.003, space="buy", optimize=True, load=True))
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        _ = metadata
        dataframe = add_pattern_continuation(dataframe, timeframe=self.timeframe, min_flag_quality=float(self.score_min.value), min_pennant_quality=float(self.score_min.value))
        return dataframe
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        condition = _bool(dataframe, "pat_pennant_pattern_present") & _num(dataframe, "pat_pennant_direction").le(-1) & _num(dataframe, "pat_pennant_indicator_score").ge(float(self.score_min.value))
...
apply_explicit_hyperopt_surface(Sieve1ContinuationPennantPresentShort1H)
