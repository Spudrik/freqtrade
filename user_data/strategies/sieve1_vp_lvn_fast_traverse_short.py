...
ENTRY_MODE = "entry_vp_lvn_fast_traverse_short"
ENTRY_TAG = "vp_lvn_fast_traverse_short"
SIDE = "short"
TRIGGER_COLUMN = "vp_node_entry_short"
CORE_BEHAVIOR = "fast downside traverse through LVN"
...
class Sieve1VpLvnFastTraverseShort(IStrategy):
    """
    Sieve1 volume-profile entry concept: fast downside traverse through LVN.
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        value_width = _num(dataframe, "vp_prior_vah", np.nan).sub(_num(dataframe, "vp_prior_val", np.nan)).abs()
        candle_range = _num(dataframe, "high").sub(_num(dataframe, "low")).abs()
        condition = (
            _bool(dataframe, TRIGGER_COLUMN)
            & _num(dataframe, "vp_score_short").gt(_num(dataframe, "vp_score_long"))
            & candle_range.ge(value_width.mul(0.20))
            & (_num(dataframe, "close").lt(_num(dataframe, "vp_prior_poc", np.nan)) | _bool(dataframe, "vp_below_value_area"))
            & _num(dataframe, "close").lt(_num(dataframe, "open"))
        )
...
apply_explicit_hyperopt_surface(Sieve1VpLvnFastTraverseShort)
