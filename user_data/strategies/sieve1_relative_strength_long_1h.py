...
ENTRY_MODE = "entry_relative_strength_long_1h"
ENTRY_TAG = "relative_strength_long_1h"
SIDE = "long"
TIMEFRAME = "1h"
...
class Sieve1RelativeStrengthLong1H(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    filter_long_caution = tagged_parameter(BooleanParameter(default=False, space="buy", optimize=True, load=True))
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe = add_relative_strength(dataframe, benchmark, short_window=int(self.short_window.value), medium_window=int(self.medium_window.value), long_window=int(self.long_window.value), min_outperformance=float(self.min_outperformance.value), percentile_window=int(self.percentile_window.value), entry_score_min=float(self.entry_score_min.value), prefix="rs")
        return dataframe
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        condition = _bool(dataframe, "rs_go_long") & _num(dataframe, "rs_score_long").ge(float(self.entry_score_min.value))
        if bool(self.filter_long_caution.value):
            condition &= ~_bool(dataframe, "rs_long_caution")
...
apply_explicit_hyperopt_surface(Sieve1RelativeStrengthLong1H)
