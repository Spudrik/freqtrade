...
ENTRY_MODE = "entry_vp_value_area_breakdown_short_1d"
ENTRY_TAG = "vp_value_area_breakdown_short_1d"
SIDE = "short"
TIMEFRAME = "1d"
...
class Sieve1VPValueAreaBreakdownShort1D(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    score_min = tagged_parameter(CategoricalParameter([0.25, 0.4, 0.5, 0.55, 0.7], default=0.5, space="buy", optimize=True, load=True))
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        _ = metadata
        dataframe = add_volume_profile(dataframe, window=int(self.window.value), bins=int(self.bins.value), value_area_pct=float(self.value_area_pct.value), price_source=str(self.price_source.value), entry_score_margin=float(self.entry_score_margin.value), node_near_pct=float(self.node_near_pct.value), pressure_delta_min=float(self.pressure_delta_min.value), volume_percentile_min=float(self.volume_percentile_min.value), prefix="vp")
        return dataframe
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        condition = _bool(dataframe, "vp_entry_trigger_short") & _num(dataframe, "vp_score_short").ge(float(self.score_min.value)) & _num(dataframe, "vp_score_short").ge(_num(dataframe, "vp_score_long")) & _bool(dataframe, "vp_below_value_area") & _num(dataframe, "close").lt(_num(dataframe, "vp_prior_val", np.nan))
...
apply_explicit_hyperopt_surface(Sieve1VPValueAreaBreakdownShort1D)
