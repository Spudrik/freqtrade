...
class Sieve1RegimePullbackShort(IStrategy):
    ...
    pullback_atr_max = tagged_parameter(
        CategoricalParameter([0.35, 0.55, 0.9], default=0.55, space="buy", optimize=True, load=True),
        "family:entries",
        "mode:entry_regime_pullback_short",
    )
    volume_ratio_min = tagged_parameter(
        CategoricalParameter([0.8, 0.85, 1.0, 1.3, 1.6], default=0.85, space="buy", optimize=True, load=True),
        "family:entries",
        "mode:entry_regime_pullback_short",
    )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if bool(self.use_vp_4h_guard.value):
            short_trigger &= self._vp_guard(dataframe, "vp4h", "short", str(self.vp_4h_guard_mode.value), float(self.vp_4h_score_min.value), float(self.vp_4h_context_min.value))
        if bool(self.use_vp_1d_guard.value):
            short_trigger &= self._vp_guard(dataframe, "vp1d", "short", str(self.vp_1d_guard_mode.value), float(self.vp_1d_score_min.value), float(self.vp_1d_context_min.value))
