...
ENTRY_MODE = "entry_reversal_inverse_head_shoulders_confirmed_long_1d"
ENTRY_TAG = "reversal_inverse_head_shoulders_confirmed_long_1d"
SIDE = "long"
TIMEFRAME = "1d"
...
class Sieve1ReversalInverseHeadShouldersConfirmedLong1D(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        _ = metadata
        dataframe = add_pattern_reversal(dataframe, timeframe=self.timeframe, min_double_quality=float(self.score_min.value))
        return dataframe
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        condition = _bool(dataframe, "pat_inverse_head_shoulders_pattern_confirmed") & _num(dataframe, "pat_inverse_head_shoulders_indicator_score").ge(float(self.score_min.value)) & _num(dataframe, "close").ge(_num(dataframe, "pat_inverse_head_shoulders_confirmation_level", np.nan))
...
apply_explicit_hyperopt_surface(Sieve1ReversalInverseHeadShouldersConfirmedLong1D)
