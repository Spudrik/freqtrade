...
ENTRY_MODE = "entry_complex_pattern_flag_breakout_long"
ENTRY_TAG = "codex_complex_pattern_break_long"
SIDE = "long"
CORE_BEHAVIOR = "pattern continuation breakout"
...
class CodexComplexPatternFlagBreakoutLong(IStrategy):
    """
    Bullish impulse/consolidation pattern that breaks upward with fresh participation.

    Single-entry Entry Sieve research strategy.
    Objective: test whether pattern continuation breakout has standalone edge under fixed +2%/-2% exits.
    """
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        pattern_ready = _num(dataframe, "pat_score_long") >= float(self.pattern_score_min.value)
        condition = (
            pattern_ready
            & (dataframe["close"] > dataframe["codex_recent_high"] * (1.0 + buffer))
            ...
            & (_bool(dataframe, "vol_vol_breakout_confirm_long") | (_num(dataframe, "vol_rvol") >= float(self.volume_rvol_min.value)))
        )
...
apply_explicit_hyperopt_surface(CodexComplexPatternFlagBreakoutLong)
