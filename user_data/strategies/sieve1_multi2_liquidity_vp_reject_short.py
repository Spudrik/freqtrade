...
ENTRY_MODE = "entry_multi2_liquidity_vp_reject_short"
ENTRY_TAG = "multi2_liquidity_vp_reject_short"
SIDE = "short"
CONCEPT = "multi2_liquidity_vp_reject"
PERIOD_KIND = "fixed"
...
class Sieve1Multi2LiquidityVpRejectShort(IStrategy):
    """
    Sieve1 entry concept: multi2_liquidity_vp_reject_short.
    ...
    breakout_buffer_pct = tagged_parameter(DecimalParameter(0.000, 0.030, decimals=3, default=0.004, space="buy", optimize=False, load=True))
    ...
    equal_level_lookback = tagged_parameter(CategoricalParameter([48, 72, 120], default=72, space="buy", optimize=True, load=True))
    ...
    multi_period = tagged_parameter(CategoricalParameter(['day', 'week', 'month'], default='day', space="buy", optimize=True, load=True))

    avwap_anchor_lookback = tagged_parameter(IntParameter(24, 336, default=120, space="buy", optimize=False, load=True))
    ...
    vp_guard_mode = tagged_parameter(CategoricalParameter(['score', 'context', 'score_or_context'], default='score_or_context', space="buy", optimize=True, load=True))
    vp_score_min = tagged_parameter(CategoricalParameter([0.15, 0.25, 0.4, 0.6], default=0.25, space="buy", optimize=True, load=True))
    vp_context_min = tagged_parameter(CategoricalParameter([0.2, 0.28, 0.45, 0.6], default=0.28, space="buy", optimize=True, load=True))
...
apply_explicit_hyperopt_surface(Sieve1Multi2LiquidityVpRejectShort)
