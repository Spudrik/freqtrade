...
ENTRY_MODE = "entry_tlv2_resistance_reject_short_1d"
ENTRY_TAG = "tlv2_resistance_reject_short_1d"
SIDE = "short"
TIMEFRAME = "1d"
...
class Sieve1TLV2ResistanceRejectShort1D(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    min_active_bars = tagged_parameter(IntParameter(2, 24, default=8, space="buy", optimize=False, load=True))
    ...
    reclaim_buffer_pct = tagged_parameter(CategoricalParameter([0.0, 0.003, 0.006, 0.012, 0.02], default=0.003, space="buy", optimize=True, load=True))
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        condition = _num(dataframe, "tlv2_resistance_score_rank0").ge(float(self.min_line_score.value)) & _num(dataframe, "tlv2_resistance_distance_atr_rank0").le(float(self.max_distance_atr.value)) & _num(dataframe, "close").le(line.mul(1.0 + float(self.reclaim_buffer_pct.value))) & _num(dataframe, "close").lt(_num(dataframe, "open"))
...
apply_explicit_hyperopt_surface(Sieve1TLV2ResistanceRejectShort1D)
