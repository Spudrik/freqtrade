...
ENTRY_MODE = "entry_geometry_ascending_channel_lower_bounce_long_1h"
ENTRY_TAG = "geometry_ascending_channel_lower_bounce_long_1h"
SIDE = "long"
TIMEFRAME = "1h"
...
class Sieve1GeometryAscendingChannelLowerBounceLong1H(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        condition = _bool(dataframe, "pg2_ascending_channel_pattern_present") & _num(dataframe, "pg2_ascending_channel_indicator_score").ge(float(self.score_min.value)) & _num(dataframe, "pg2_ascending_channel_width_atr", np.nan).le(float(self.width_atr_max.value)) & _num(dataframe, "low").le(lower.mul(1.0 + float(self.rail_buffer_pct.value))) & _num(dataframe, "close").gt(lower) & _num(dataframe, "close").gt(_num(dataframe, "open"))
...
apply_explicit_hyperopt_surface(Sieve1GeometryAscendingChannelLowerBounceLong1H)
