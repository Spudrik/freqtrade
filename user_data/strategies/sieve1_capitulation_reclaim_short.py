...
class Sieve1CapitulationReclaimShort(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if bool(self.use_vp_4h_guard.value):
            short_reclaim &= self._vp_guard(dataframe, "vp4h", "short", str(self.vp_4h_guard_mode.value), float(self.vp_4h_score_min.value), float(self.vp_4h_context_min.value))
        if bool(self.use_vp_1d_guard.value):
            short_reclaim &= self._vp_guard(dataframe, "vp1d", "short", str(self.vp_1d_guard_mode.value), float(self.vp_1d_score_min.value), float(self.vp_1d_context_min.value))
