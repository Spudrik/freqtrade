...
ENTRY_MODE = "entry_triple_bottom_confirmation_pressure_long_8h"
ENTRY_TAG = "triple_bottom_confirmation_pressure_long_8h"
SIDE = "long"
TIMEFRAME = "8h"
...
class Sieve1TripleBottomConfirmationPressureLong8H(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    confirmation_buffer_pct = tagged_parameter(CategoricalParameter([0.0, 0.002, 0.005, 0.01], default=0.002, space="buy", optimize=True, load=True))
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        _ = metadata
        dataframe = add_pattern_peaks(dataframe, timeframe=self.timeframe, min_triple_quality=float(self.score_min.value))
        return dataframe
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        level = _num(dataframe, "pat_triple_bottom_confirmation_level", np.nan).mul(1.0 + float(self.confirmation_buffer_pct.value))
        condition = _bool(dataframe, "pat_triple_bottom_pattern_present") & _num(dataframe, "pat_triple_bottom_indicator_score").ge(float(self.score_min.value)) & _cross_above(_num(dataframe, "close"), level)
...
apply_explicit_hyperopt_surface(Sieve1TripleBottomConfirmationPressureLong8H)
