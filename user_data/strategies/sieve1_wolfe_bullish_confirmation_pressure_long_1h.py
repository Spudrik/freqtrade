...
ENTRY_MODE = "entry_wolfe_bullish_confirmation_pressure_long_1h"
ENTRY_TAG = "wolfe_bullish_confirmation_pressure_long_1h"
SIDE = "long"
TIMEFRAME = "1h"
...
class Sieve1WolfeBullishConfirmationPressureLong1H(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        level = _num(dataframe, "pww_bullish_confirmation_level", np.nan).mul(1.0 + float(self.confirmation_buffer_pct.value))
        condition = _bool(dataframe, "pww_bullish_pattern_present") & _num(dataframe, "pww_bullish_indicator_score").ge(float(self.score_min.value)) & _cross_above(_num(dataframe, "close"), level)
...
apply_explicit_hyperopt_surface(Sieve1WolfeBullishConfirmationPressureLong1H)
