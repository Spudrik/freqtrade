...
ENTRY_MODE = "entry_geometry_rectangle_breakdown_short_1d"
ENTRY_TAG = "geometry_rectangle_breakdown_short_1d"
SIDE = "short"
TIMEFRAME = "1d"
...
class Sieve1GeometryRectangleBreakdownShort1D(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        lower = _num(dataframe, "pg2_rectangle_lower", np.nan).mul(1.0 - float(self.rail_buffer_pct.value))
        condition = _bool(dataframe, "pg2_rectangle_pattern_present") & _num(dataframe, "pg2_rectangle_indicator_score").ge(float(self.score_min.value)) & _num(dataframe, "pg2_rectangle_width_atr", np.nan).le(float(self.width_atr_max.value)) & _cross_below(_num(dataframe, "close"), lower)
...
apply_explicit_hyperopt_surface(Sieve1GeometryRectangleBreakdownShort1D)
