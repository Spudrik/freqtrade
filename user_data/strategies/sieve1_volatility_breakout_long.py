...
class Sieve1VolatilityBreakoutLong(IStrategy):
    """
    ...
    channel_period = tagged_parameter(
        ...
        "family:entries",
        "mode:entry_volatility_breakout_long",
    )
    ...
    atr_compression_max = tagged_parameter(
        CategoricalParameter([0.55, 0.75, 1.0], default=0.75, space="buy", optimize=True, load=True),
        "family:entries",
        "mode:entry_volatility_breakout_long",
    )
    breakout_buffer_pct = tagged_parameter(
        CategoricalParameter([0.0, 0.002, 0.003, 0.006, 0.012], default=0.002, space="buy", optimize=True, load=True),
        "family:entries",
        "mode:entry_volatility_breakout_long",
    )
    volume_ratio_min = tagged_parameter(
        CategoricalParameter([0.8, 1.0, 1.3, 1.35, 1.6], default=1.35, space="buy", optimize=True, load=True),
        "family:entries",
        "mode:entry_volatility_breakout_long",
    )
    close_location_min = tagged_parameter(
        CategoricalParameter([0.6, 0.68, 0.8], default=0.68, space="buy", optimize=True, load=True),
        "family:entries",
        "mode:entry_volatility_breakout_long",
    )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if bool(self.use_vp_4h_guard.value):
            long_breakout &= self._vp_guard(dataframe, "vp4h", "long", str(self.vp_4h_guard_mode.value), float(self.vp_4h_score_min.value), float(self.vp_4h_context_min.value))
        if bool(self.use_vp_1d_guard.value):
            long_breakout &= self._vp_guard(dataframe, "vp1d", "long", str(self.vp_1d_guard_mode.value), float(self.vp_1d_score_min.value), float(self.vp_1d_context_min.value))

        dataframe.loc[long_breakout.fillna(False), "enter_long"] = 1
        dataframe.loc[long_breakout.fillna(False), "enter_tag"] = "vol_breakout_long"
        return dataframe
...
apply_explicit_hyperopt_surface(Sieve1VolatilityBreakoutLong)
