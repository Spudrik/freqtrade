...
class CodexComplexVolumeProfileValueReclaimLong(IStrategy):
    ...
    Objective: test whether volume profile value reclaim has standalone edge under fixed +2%/-2% exits.
    """
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        profile_event = _bool(dataframe, "vp_lower_rejection_with_pressure") | _bool(dataframe, "vp_hvn_below_reclaim") | _bool(dataframe, "vp_lvn_accept_long")
        condition = (
            profile_event
            & (dataframe["close"] > _num(dataframe, "vp_poc", dataframe["close"]))
            & (_num(dataframe, "vp_score_long") >= float(self.profile_score_min.value))
            & (_num(dataframe, "vol_score_long") >= float(self.volume_score_min.value))
