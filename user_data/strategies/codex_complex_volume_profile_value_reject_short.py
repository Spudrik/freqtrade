...
ENTRY_MODE = "entry_complex_volume_profile_value_reject_short"
ENTRY_TAG = "codex_complex_vp_reject_short"
SIDE = "short"
CORE_BEHAVIOR = "volume profile value rejection"
...
class CodexComplexVolumeProfileValueRejectShort(IStrategy):
    """
    Failed auction above value/HVN rejection with bearish profile and volume pressure.

    Single-entry Entry Sieve research strategy.
    Objective: test whether volume profile value rejection has standalone edge under fixed +2%/-2% exits.
    """
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        profile_event = _bool(dataframe, "vp_upper_rejection_with_pressure") | _bool(dataframe, "vp_hvn_above_reject") | _bool(dataframe, "vp_lvn_accept_short")
        condition = (
            profile_event
            & (dataframe["close"] < _num(dataframe, "vp_poc", dataframe["close"]))
            & (_num(dataframe, "vp_score_short") >= float(self.profile_score_min.value))
            & (_num(dataframe, "vol_score_short") >= float(self.volume_score_min.value))
...
apply_explicit_hyperopt_surface(CodexComplexVolumeProfileValueRejectShort)
