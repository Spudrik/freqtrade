...
ENTRY_MODE = "entry_triple_top_confirmation_pressure_short_1h"
ENTRY_TAG = "triple_top_confirmation_pressure_short_1h"
SIDE = "short"
TIMEFRAME = "1h"
...
class Sieve1TripleTopConfirmationPressureShort1H(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        level = _num(dataframe, "pat_triple_top_confirmation_level", np.nan).mul(1.0 - float(self.confirmation_buffer_pct.value))
        condition = _bool(dataframe, "pat_triple_top_pattern_present") & _num(dataframe, "pat_triple_top_indicator_score").ge(float(self.score_min.value)) & _cross_below(_num(dataframe, "close"), level)
...
apply_explicit_hyperopt_surface(Sieve1TripleTopConfirmationPressureShort1H)
