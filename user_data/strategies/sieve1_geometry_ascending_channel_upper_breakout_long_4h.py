...
ENTRY_MODE = "entry_geometry_ascending_channel_upper_breakout_long_4h"
ENTRY_TAG = "geometry_ascending_channel_upper_breakout_long_4h"
SIDE = "long"
TIMEFRAME = "4h"
...
class Sieve1GeometryAscendingChannelUpperBreakoutLong4H(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        upper = _num(dataframe, "pg2_ascending_channel_upper", np.nan).mul(1.0 + float(self.rail_buffer_pct.value))
        condition = _bool(dataframe, "pg2_ascending_channel_pattern_present") & _num(dataframe, "pg2_ascending_channel_indicator_score").ge(float(self.score_min.value)) & _num(dataframe, "pg2_ascending_channel_width_atr", np.nan).le(float(self.width_atr_max.value)) & _cross_above(_num(dataframe, "close"), upper)
...
apply_explicit_hyperopt_surface(Sieve1GeometryAscendingChannelUpperBreakoutLong4H)
