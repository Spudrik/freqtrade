...
ENTRY_MODE = "entry_geometry_triangle_lower_reclaim_long_1h"
ENTRY_TAG = "geometry_triangle_lower_reclaim_long_1h"
SIDE = "long"
TIMEFRAME = "1h"
...
class Sieve1GeometryTriangleLowerReclaimLong1H(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = TIMEFRAME
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        lower = _num(dataframe, "pg2_triangle_lower", np.nan)
        condition = _bool(dataframe, "pg2_triangle_pattern_present") & _num(dataframe, "pg2_triangle_indicator_score").ge(float(self.score_min.value)) & _num(dataframe, "pg2_triangle_width_atr", np.nan).le(float(self.width_atr_max.value)) & _num(dataframe, "low").le(lower.mul(1.0 + float(self.rail_buffer_pct.value))) & _num(dataframe, "close").gt(lower) & _num(dataframe, "close").gt(_num(dataframe, "open"))
...
apply_explicit_hyperopt_surface(Sieve1GeometryTriangleLowerReclaimLong1H)
