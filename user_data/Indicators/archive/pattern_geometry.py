...
def _triangle_wedge_arrays(frame: DataFrame, cfg: PatternStructureConfig) -> dict[str, np.ndarray]:
    ...
    window, min_bars = _effective_geometry_window(cfg)
    ...
    for row in range(min_span, rows):
        ...
        for start_anchor in _geometry_envelope_candidate_starts(pivot_events, row, min_span, window):
...
def _geometry_envelope_candidate_starts(
    ...
    return np.unique(pivot_events[(pivot_events >= lower) & (pivot_events <= upper)].astype("float64"))
...
def _effective_geometry_window(cfg: PatternStructureConfig) -> tuple[int, int]:
    return int(cfg.triangle_window), int(cfg.min_triangle_bars)
