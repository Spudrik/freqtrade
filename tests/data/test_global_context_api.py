...
def test_global_context_normalizers_score_and_note_payloads() -> None:
    ...
    global_rows = normalize_coingecko_global(
    ...
    assert global_row["source_score"] is None
    assert global_row["calc_score"] == 60.0
...
def test_defillama_normalizers_build_liquidity_context() -> None:
    ...
    assert stablecoins["calc_score"] == stablecoins["score"]
    assert "supply" in stablecoins["notes"]
    supply = next(row for row in stablecoin_rows if row["metric_key"] == "stablecoin_supply_usd")
    ...
    assert chains["calc_score"] == chains["score"]
...
def test_service_command_and_latest_rows(tmp_path: Path) -> None:
    ...
    state = {
        ...
        "key_file": str(tmp_path / "frequi_keys.json"),
        "fred_key_json_path": "fred.api_key",
        "enable_fred": True,
        "interval_minutes": "30",
        "once": True,
    }
    ...
    assert "--enable-fred" in command
    assert command[command.index("--key-file") + 1] == str(tmp_path / "frequi_keys.json")
    assert command[command.index("--fred-key-json-path") + 1] == "fred.api_key"
...
def test_fred_key_file_json_path(tmp_path: Path) -> None:
    key_file = tmp_path / "frequi_keys.json"
    key_file.write_text('{"fred": {"api_key": "abc123"}}', encoding="utf-8")
    assert load_fred_api_key_from_file(key_file, "fred.api_key") == "abc123"
    assert load_fred_api_key_from_file(key_file, "missing.path") == "abc123"
